...
const request = async (endpoint, options = {}) => {
  ...
  console.log('🔑 Token atual:', token ? `${token.substring(0, 30)}...` : 'NENHUM TOKEN');
...
export const api = {
  ...
  createAluno: (data) => request('/admin/alunos', { 
    method: 'POST', 
    body: JSON.stringify(data) 
  }),
  updateAluno: (id, data) => request(`/admin/alunos/${id}`, { 
    method: 'PUT', 
    body: JSON.stringify(data) 
  }),
  ...
  createProfessor: (data) => request('/admin/professores', { 
    method: 'POST', 
    body: JSON.stringify(data) 
  }),
  updateProfessor: (id, data) => request(`/admin/professores/${id}`, { 
    method: 'PUT', 
    body: JSON.stringify(data) 
  }),
  ...
  createTurma: (data) => request('/turmas', { 
    method: 'POST', 
    body: JSON.stringify(data) 
  }),
  updateTurma: (id, data) => request(`/turmas/${id}`, { 
    method: 'PUT', 
    body: JSON.stringify(data) 
  }),
  deleteTurma: (id) => request(`/turmas/${id}`, { 
    method: 'DELETE' 
  }),
  
  // Admin - Pontos de Acesso
  getAPs: () => request('/admin/aps'),
  createAP: (data) => request('/admin/aps', { 
    method: 'POST', 
    body: JSON.stringify(data) 
  }),
  ...
  registrarPresencaProfessor: (data) => request('/professor/presenca', { 
    method: 'POST', 
    body: JSON.stringify(data) 
  }),
  ...
  registrarPresencaAuto: async (data) => {
    console.log('📡 Enviando presença automática:', data);
    try {
      const result = await request('/presenca/auto', { 
        method: 'POST', 
        body: JSON.stringify(data) 
      });
      console.log('✅ Resposta presença automática:', result);
      return result;
    } catch (error) {
      console.error('❌ Erro na presença automática:', error);
      throw error;
    }
  },
