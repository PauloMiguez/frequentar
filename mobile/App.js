...
const App = () => {
  ...
  const getDeviceId = async () => {
    try {
      ...
      if (!id) {
        ...
        console.log('��� Device ID existente:', id);
      }
      setDeviceId(id);
      return id;
    } catch (error) {
  ...
  const startWifiMonitoring = () => {
    console.log('��� Iniciando monitoramento de Wi-Fi...');
    
    const unsubscribe = NetInfo.addEventListener(async state => {
      console.log('��� Estado da rede:', { type: state.type, isConnected: state.isConnected });
      
      if (state.isConnected && state.type === 'wifi') {
        console.log('✅ Conectado ao Wi-Fi!');
        const deviceId_local = await getDeviceId();
        console.log('��� Device ID atual:', deviceId_local);
        if (deviceId_local) {
  ...
  const registerPresenceByWifi = async (deviceIdentifier) => {
    try {
      console.log('��� Enviando requisição para:', `${API_URL}/presenca/wifi`);
      ...
      if (data.status === 'registrado') {
        ...
        console.log('⚠️ Presença já registrada hoje');
      } else if (data.error === 'Dispositivo não cadastrado') {
        console.log('❌ Device ID não cadastrado no sistema:', deviceIdentifier);
      } else {
        console.log('❌ Falha no registro:', data);
      }
    } catch (error) {
  ...
  const loadWifiConfig = async () => {
    try {
      ...
      if (data.ssid) {
        ...
        console.log('✅ Configuração Wi-Fi carregada:', data.ssid);
      }
    } catch (error) {
  ...
  const saveWifiConfig = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('✅ Configuração salva:', wifiConfig);
      } else {
  ...
  const loadDevices = async () => {
    try {
      ...
      const data = await response.json();
      setDevices(data);
      console.log('��� Dispositivos carregados:', data.length);
    } catch (error) {
  ...
  const renderHistoryItem = ({ item }) => (
    <View style={styles.historyItem}>
      ...
      <Text style={styles.historyInfo}>Matrícula: {item.matricula}</Text>
      <Text style={styles.historyInfo}>Data: {formatDate(item.data_conexao)}</Text>
      <Text style={styles.historyInfo}>Horário: {item.hora_conexao}</Text>
    ...
    return (
      <View style={styles.container}>
        <View style={styles.loginCard}>
          <Text style={styles.title}>Sistema de Presença Wi-Fi</Text>
          <TextInput
            style={styles.input}
            placeholder="Usuário"
  ...
  return (
    <View style={styles.adminContainer}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Presença Wi-Fi</Text>
      ...
      >
        {activeTab === 'dashboard' && (
          <View>
            ...
            <View style={styles.section}>
              ...
              <Text style={styles.wifiInfo}>SSID: {wifiConfig.ssid || 'Não configurado'}</Text>
            </View>
              ...
              {devices.map((device) => (
                <View key={device.id} style={styles.deviceCard}>
                  <View style={styles.deviceInfo}>
                    ...
                    <Text style={styles.deviceDetail}>Matrícula: {device.matricula}</Text>
        ...
        {activeTab === 'settings' && (
          <View>
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>Configuração da Rede</Text>
              <TextInput
                style={styles.input}
